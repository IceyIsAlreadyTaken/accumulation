...
function initRecorder(stream) {
  ...
  const audioInput = audioContext.createMediaStreamSource(stream); // 缓冲区大小为4096，控制着多长时间需触发一次audioprocess事件
  ...
  recorder.onaudioprocess = function recorderProcess() {
    console.log('123');
  };
  ...
  audioInput.connect(monitorGainNode); // const recordingGainNode = audioContext.createGain()
  ...
  audioInput.connect(recorder); // 最后先音频流输出到扬声器。（将录音流原本的输出位置再定回原来的目标地）
